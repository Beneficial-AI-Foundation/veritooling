...
| `secure-messaging/` | leanblueprint | `progress_history.py --pipeline leanblueprint --since 2026-06-24 --cadence weekly --branch origin/main`; sampled off `main`, so pass `--branch` — the default ref is the work-clone's `origin/HEAD`, i.e. whatever branch the source checkout was on. No `--verso-render-cmd` from `a7d1690` (2026-09-07) on: the repo's root `BlueprintMain.lean` makes probe-leanblueprint's default `lake exe vbp build` work. Earlier commits still need `--verso-render-cmd scripts/render-docs-site.sh`. `depgraph.*`/`insights.txt` from one commit's extract | Chart: yes; step-by-step in [`secure-messaging/update-progress.md`](secure-messaging/update-progress.md). Graph/insights: need an extract (not committed). |

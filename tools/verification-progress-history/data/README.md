...
| `secure-messaging/` | leanblueprint | `progress_history.py --pipeline leanblueprint --since 2026-06-24 --cadence weekly --branch origin/main` (plus `--verso-render-cmd`); sampled off `main`, so pass `--branch` — the default ref is the work-clone's `origin/HEAD`, i.e. whatever branch the source checkout was on. `depgraph.*`/`insights.txt` from one commit's extract | Chart: yes; step-by-step in [`secure-messaging/update-progress.md`](secure-messaging/update-progress.md). Graph/insights: need an extract (not committed). |

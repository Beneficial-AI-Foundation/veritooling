...
These reusable workflows build a Lean 4 project and turn its `sorry`/axiom state
into a pull-request comment. Reference them from your own repository with
...
The two run as separate jobs by design. The build runs untrusted PR code with no
write access; the comment job, which needs `pull-requests: write`, only ever sees
the artifacts the build produced. This lets fork PRs get comments without
exposing a write token to untrusted code.
...
2. **Audit**: one of two backends scans the build and writes `sorry-manifest.txt`,
   a versioned list of every sorry-tainted declaration.
   - `collectaxioms`: a Lean metaprogram that walks the transitive axiom closure
     of each declaration (catching `sorry` reached indirectly, plus
     `Lean.trustCompiler` and custom axioms). Also prints the console report below.
...
4. **Report**: `sorry-delta` diffs head against base; `specs-delta` optionally
   ...
   combines them into one sticky **Verification Delta** comment.
...
4. **Sorry manifest**: the machine-readable `sorry-manifest.txt` that feeds the
   delta; lists all sorry-tainted declarations (theorems and definitions).

Two scopes to keep straight. Sections 1 and 2 are what is audited in detail: by
default all project modules, narrowed by `exclude-module`, counting theorems and
axioms (your claims). Sections 3 and 4 cover the full project: always every
module and every declaration kind. So the manifest count is usually higher than
Section 2's, since it includes unfinished definitions, not just tainted theorems.
...
On projects that carry `sorry`s in generated/extracted code, gate the comment on
hand-written modules so it only fires when a new hand-written `sorry` appears:
...
`lean-build-audit.yml` is just `checkout` plus `lake build` plus the
...
that action into your own job instead of calling the whole workflow.
